...
export async function exportBundle(p: Project): Promise<{ bytes: Uint8Array; pages: number; memoryMB: number }> {
  ...
  entries.push(textEntry(`${stem}.project.json`, JSON.stringify(p)), textEntry('IMPORT.md', importGuide(stem)),
    ...
    textEntry('compatibility.json', JSON.stringify({ exporter: 'ai-bone-studio/0.3.0', format: 'spine-3.8.99-region-fk-attachment', alpha: 'straight', schemaPreflight: 'passed', engineIntegrationTest: 'not-run-in-user-project', targetRuntime: '3.8', estimatedTextureMemoryMB: memoryMB, issues: preflight(p) }, null, 2)));
